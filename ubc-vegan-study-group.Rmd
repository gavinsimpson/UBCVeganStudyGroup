---
title: "Community Data Analysis Using Vegan"
...
Handouts and all the sources are on [github](https://github.com/gavinsimpson/UBCVeganStudyGroup):

[github.com/gavinsimpson/UBCVeganStudyGroup](https://github.com/gavinsimpson/UBCVeganStudyGroup)
...
Finds axes that are **linear** combinations of the explanatory variables
...
`summary()` method for `eigenvals()` shows more information
...
From vegan version `2.3-1` we have a simpler way to select the type of scaling

`scaling` can be a character vector indicating the broad type of scaling required
...
A second argument then allows selection of the relevant negative scaling type:

* `correlation = TRUE` for linear methods
* `hill = TRUE` for unimodal methods
...
\scriptsize
```{r triplot-2, fig.height = 6, crop.plot = TRUE, out.width = "0.55\\linewidth"}
plot(cca1)
```
\normalsize
...
The object returned by `ordistep()` is a standard `"cca"` object with an extra component `$anova`
 ...
 * If no conditioning (partial) variables then rows of the species data are permuted
...
## Details on the permutations --- `permustats`

To get details on the permutations use `permustats()`
...
\scriptsize
```{r permustats-2, fig.show = "hold", fig.width = 10, fig.height = 5}
...
\scriptsize
```{r anova-args, echo = FALSE}
args(anova.cca)
```
\normalsize

* `permutations` controls the permutation test; one of
    1. a number of (unrestricted) permutations to do
    2. a matrix of pre-generated permutations
    3. an object returned by `how()`
* `by` determines what is tested; the default is to test the model
* More direct control can be achieved via `permutest.cca()`
* `parallel` allows for running the permutations on multiple CPU cores
...
Can *randomly* link starting point of one series to any time point of another series if series are stationary under null hypothesis that the series are unrelated
...
(Without wanting to get *too* technical) Mirroring in restricted permutations allows for isotropy in dependencies by reflecting the ordering of samples in time or spatial dimensions
...
\scriptsize
```{r worked-example-devel-2a, dependson = -1}
set.seed(23)
...
\scriptsize
```{r worked-example-devel-2b, dependson = -2}
set.seed(24)
...
## Hierarchical analysis of crayfish --- Unconstrained
\scriptsize
```{r crayfish-pca-plot, fig.width = 16, fig.height = 7.5, fig.show = "hold"}
layout(matrix(1:2, ncol = 2))
...
Copyright \textcopyright (2015) Gavin L. Simpson *Some Rights Reserved*
