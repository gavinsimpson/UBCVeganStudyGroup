...
## ----eigenvals-2---------------------------------------------------------
summary(eigenvals(cca1, constrained = TRUE))
...
## ----stepwise-reverse----------------------------------------------------
mods2 <- ordistep(upr, scope = list(lower = formula(lwr), upper = formula(upr)), trace = 0)
mods2
...
## ----anova-by-axis-------------------------------------------------------
set.seed(1)
anova(mods, by = "axis", permutations = 499)

## ----anova-by-term-------------------------------------------------------
set.seed(5)
anova(mods, by = "terms", permutations = 499)

## ----anova-by-margin-----------------------------------------------------
set.seed(10)
anova(mods, by = "margin", permutations = 499)
...
m2 <- ordistep(lwr, scope = formula(m1), trace = FALSE, permutations = 99)
m2
...
m4 <- ordistep(lwr, scope = formula(m3), trace = FALSE, permutations = 99)

## ----meadows-rda-reduced-triplot, fig.height = 5, crop.plot = TRUE, out.width = "0.5\\linewidth"----
plot(m4)

## ----meadows-rda-adjrsquare----------------------------------------------
m5 <- ordiR2step(lwr, scope = formula(m3), trace = FALSE, permutations = 99)
m5$anova
...
## ----crayfish-pca-plot, fig.width = 16, fig.height = 7.5, fig.show = "hold"----
layout(matrix(1:2, ncol = 2))
