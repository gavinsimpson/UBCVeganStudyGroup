...
\title{Community Data Analysis Using Vegan}
\author{Gavin L. Simpson}
\date{UBC Study Group • Nov 25th 2015}
...
Handouts and all the sources are on
\href{https://github.com/gavinsimpson/UBCVeganStudyGroup}{github}:

\href{https://github.com/gavinsimpson/UBCVeganStudyGroup}{github.com/gavinsimpson/UBCVeganStudyGroup}
...
Finds axes that are \textbf{linear} combinations of the explanatory
variables
...
\texttt{summary()} method for \texttt{eigenvals()} shows more
information
...
\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{summary}\NormalTok{(}\KeywordTok{eigenvals}\NormalTok{(cca1, }\DataTypeTok{constrained =} \OtherTok{TRUE}\NormalTok{))}
\end{Highlighting}
\end{Shaded}
...
Eigenvalue            0.05836 0.03114 0.01329 0.008364 0.006538 0.006156
Proportion Explained  0.04049 0.02160 0.00922 0.005800 0.004540 0.004270
Cumulative Proportion 0.95128 0.97288 0.98211 0.987910 0.992450 0.996720
                         CCA14
Eigenvalue            0.004733
Proportion Explained  0.003280
...
From vegan version \texttt{2.3-1} we have a simpler way to select the
type of scaling

\texttt{scaling} can be a character vector indicating the broad type of
scaling required
...
A second argument then allows selection of the relevant negative scaling
type:

\begin{itemize}
\itemsep1pt\parskip0pt\parsep0pt
\item
  \texttt{correlation\ =\ TRUE} for linear methods
\item
  \texttt{hill\ =\ TRUE} for unimodal methods
\end{itemize}
...
\begin{center}\includegraphics[width=0.5\linewidth]{ubc-vegan-study-group_files/figure-beamer/triplot-1-1} \end{center}
...
\begin{center}\includegraphics[width=0.55\linewidth]{ubc-vegan-study-group_files/figure-beamer/triplot-2-1} \end{center}
...
The object returned by \texttt{ordistep()} is a standard \texttt{"cca"}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{mods2 <-}\StringTok{ }\KeywordTok{ordistep}\NormalTok{(upr, }\DataTypeTok{scope =} \KeywordTok{list}\NormalTok{(}\DataTypeTok{lower =} \KeywordTok{formula}\NormalTok{(lwr), }\DataTypeTok{upper =} \KeywordTok{formula}\NormalTok{(upr)), }\DataTypeTok{trace =} \DecValTok{0}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\begin{verbatim}
Call: cca(formula = varespec ~ P + K + S + Mo + Baresoil +
Humdepth, data = varechem)

              Inertia Proportion Rank
Total          2.0832     1.0000     
Constrained    0.9377     0.4501    6
Unconstrained  1.1455     0.5499   17
Inertia is mean squared contingency coefficient 

Eigenvalues for constrained axes:
  CCA1   CCA2   CCA3   CCA4   CCA5   CCA6 
0.3712 0.2356 0.1364 0.0974 0.0813 0.0159 

Eigenvalues for unconstrained axes:
   CA1    CA2    CA3    CA4    CA5    CA6    CA7    CA8 
0.3295 0.1893 0.1548 0.1269 0.0968 0.0616 0.0464 0.0337 
(Showed only 8 of all 17 unconstrained eigenvalues)
\end{verbatim}
...
\itemsep1pt\parskip0pt\parsep0pt
\item
  If no conditioning (partial) variables then rows of the species data
  are permuted
\item
...
\NormalTok{(perm <-}\StringTok{ }\KeywordTok{anova}\NormalTok{(cca1, }\DataTypeTok{permutations =} \DecValTok{499}\NormalTok{))}
\end{Highlighting}
\end{Shaded}
...
\begin{frame}[fragile]{Details on the permutations ---
\texttt{permustats}}

To get details on the permutations use \texttt{permustats()}

\scriptsize

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{pstat <-}\StringTok{ }\KeywordTok{permustats}\NormalTok{(perm)}
\KeywordTok{summary}\NormalTok{(pstat)}
\end{Highlighting}
\end{Shaded}
...
Model    1.4441 1.9675 1.0513       1.0266 1.4190
...
\KeywordTok{plot}\NormalTok{(}\KeywordTok{density}\NormalTok{(pstat))}
\KeywordTok{qqnorm}\NormalTok{(pstat)}
\KeywordTok{layout}\NormalTok{(}\DecValTok{1}\NormalTok{)}
\end{Highlighting}
\end{Shaded}

\begin{center}\includegraphics[width=0.7\linewidth]{ubc-vegan-study-group_files/figure-beamer/permustats-2-1} \end{center}
...
\itemsep1pt\parskip0pt\parsep0pt
\item
  \texttt{permutations} controls the permutation test; one of

  \begin{enumerate}
  \def\labelenumi{\arabic{enumi}.}
  \itemsep1pt\parskip0pt\parsep0pt
  \item
    a number of (unrestricted) permutations to do
  \item
    a matrix of pre-generated permutations
  \item
    an object returned by \texttt{how()}
  \end{enumerate}
\item
...
\item
  \texttt{parallel} allows for running the permutations on multiple CPU
  cores
\end{itemize}
...
\begin{Highlighting}[]
\KeywordTok{set.seed}\NormalTok{(}\DecValTok{1}\NormalTok{)}
\KeywordTok{anova}\NormalTok{(mods, }\DataTypeTok{by =} \StringTok{"axis"}\NormalTok{, }\DataTypeTok{permutations =} \DecValTok{499}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
Model: cca(formula = varespec ~ Al + P + K, data = varechem)
         Df ChiSquare      F Pr(>F)   
CCA1      1   0.36156 5.0249  0.002 **
CCA2      1   0.16996 2.3621  0.014 * 
CCA3      1   0.11262 1.5651  0.114   
Residual 20   1.43906                 
---
Signif. codes:  0 '***' 0.001 '**' 0.01 '*' 0.05 '.' 0.1 ' ' 1
\end{verbatim}
...
\KeywordTok{anova}\NormalTok{(mods, }\DataTypeTok{by =} \StringTok{"terms"}\NormalTok{, }\DataTypeTok{permutations =} \DecValTok{499}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
Model: cca(formula = varespec ~ Al + P + K, data = varechem)
         Df ChiSquare      F Pr(>F)   
Al        1   0.29817 4.1440  0.002 **
P         1   0.18991 2.6393  0.012 * 
K         1   0.15605 2.1688  0.030 * 
Residual 20   1.43906                 
---
Signif. codes:  0 '***' 0.001 '**' 0.01 '*' 0.05 '.' 0.1 ' ' 1
\end{verbatim}
...
\KeywordTok{anova}\NormalTok{(mods, }\DataTypeTok{by =} \StringTok{"margin"}\NormalTok{, }\DataTypeTok{permutations =} \DecValTok{499}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
Model: cca(formula = varespec ~ Al + P + K, data = varechem)
         Df ChiSquare      F Pr(>F)   
Al        1   0.31184 4.3340  0.002 **
P         1   0.16810 2.3362  0.014 * 
K         1   0.15605 2.1688  0.032 * 
Residual 20   1.43906                 
---
Signif. codes:  0 '***' 0.001 '**' 0.01 '*' 0.05 '.' 0.1 ' ' 1
\end{verbatim}
...
\KeywordTok{anova}\NormalTok{(m1, }\DataTypeTok{permutations =} \DecValTok{499}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\begin{center}\includegraphics[width=0.5\linewidth]{ubc-vegan-study-group_files/figure-beamer/meadows-cca-full-triplot-1} \end{center}
...
\NormalTok{m2 <-}\StringTok{ }\KeywordTok{ordistep}\NormalTok{(lwr, }\DataTypeTok{scope =} \KeywordTok{formula}\NormalTok{(m1), }\DataTypeTok{trace =} \OtherTok{FALSE}\NormalTok{, }\DataTypeTok{permutations =} \DecValTok{99}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\begin{verbatim}
Call: cca(formula = spp ~ Ca + conduct + Corg + NH3 + Si, data =
env)

              Inertia Proportion Rank
Total          5.3107     1.0000     
Constrained    0.8034     0.1513    5
Unconstrained  4.5073     0.8487   64
Inertia is mean squared contingency coefficient 

Eigenvalues for constrained axes:
  CCA1   CCA2   CCA3   CCA4   CCA5 
0.4221 0.1195 0.1036 0.0807 0.0775 

Eigenvalues for unconstrained axes:
    CA1     CA2     CA3     CA4     CA5     CA6     CA7     CA8 
0.29763 0.19604 0.17163 0.15357 0.14613 0.14229 0.13519 0.12476 
(Showed only 8 of all 64 unconstrained eigenvalues)
\end{verbatim}
...
\begin{center}\includegraphics[width=0.5\linewidth]{ubc-vegan-study-group_files/figure-beamer/meadows-cca-reduced-triplot-1} \end{center}
...
\begin{verbatim}
          Df    AIC      F Pr(>F)   
+ Ca       1 453.14 4.7893   0.01 **
+ conduct  1 453.29 1.7915   0.01 **
+ Corg     1 453.61 1.6011   0.01 **
+ NH3      1 453.96 1.5533   0.01 **
+ Si       1 454.42 1.4231   0.01 **
---
Signif. codes:  0 '***' 0.001 '**' 0.01 '*' 0.05 '.' 0.1 ' ' 1
\end{verbatim}
...
\NormalTok{m4 <-}\StringTok{ }\KeywordTok{ordistep}\NormalTok{(lwr, }\DataTypeTok{scope =} \KeywordTok{formula}\NormalTok{(m3), }\DataTypeTok{trace =} \OtherTok{FALSE}\NormalTok{, }\DataTypeTok{permutations =} \DecValTok{99}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\begin{center}\includegraphics[width=0.5\linewidth]{ubc-vegan-study-group_files/figure-beamer/meadows-rda-reduced-triplot-1} \end{center}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{m5 <-}\StringTok{ }\KeywordTok{ordiR2step}\NormalTok{(lwr, }\DataTypeTok{scope =} \KeywordTok{formula}\NormalTok{(m3), }\DataTypeTok{trace =} \OtherTok{FALSE}\NormalTok{, }\DataTypeTok{permutations =} \DecValTok{99}\NormalTok{)}
\NormalTok{m5$anova}
\end{Highlighting}
\end{Shaded}
...
+ Ca            0.12588  1 -41.779 10.9370   0.01 **
+ NH3           0.14628  1 -42.468  2.6242   0.01 **
+ conduct       0.16322  1 -42.925  2.3570   0.01 **
+ Si            0.17711  1 -43.164  2.1136   0.01 **
+ Corg          0.18518  1 -42.940  1.6442   0.01 **
+ NO3           0.19257  1 -42.680  1.5853   0.02 * 
+ pH            0.19966  1 -42.417  1.5583   0.01 **
...
Can \emph{randomly} link starting point of one series to any time point
...
permutations allows for isotropy in dependencies by reflecting the
...
\NormalTok{env <-}\StringTok{ }\KeywordTok{transform}\NormalTok{(env, }\DataTypeTok{year =} \KeywordTok{as.numeric}\NormalTok{(}\KeywordTok{as.character}\NormalTok{(year))) }\CommentTok{# Year as numeric}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{c1 <-}\StringTok{ }\KeywordTok{rda}\NormalTok{(spp ~}\StringTok{ }\NormalTok{year +}\StringTok{ }\NormalTok{year:mowing +}\StringTok{ }\NormalTok{year:fertilizer +}\StringTok{ }\NormalTok{year:removal +}
\StringTok{              }\KeywordTok{Condition}\NormalTok{(plotid), }\DataTypeTok{data =} \NormalTok{env)}
\NormalTok{(h <-}\StringTok{ }\KeywordTok{how}\NormalTok{(}\DataTypeTok{nperm =} \DecValTok{499}\NormalTok{, }\DataTypeTok{within =} \KeywordTok{Within}\NormalTok{(}\DataTypeTok{type =} \StringTok{"none"}\NormalTok{),}
          \DataTypeTok{plots =} \KeywordTok{with}\NormalTok{(env, }\KeywordTok{Plots}\NormalTok{(}\DataTypeTok{strata =} \NormalTok{plotid, }\DataTypeTok{type =} \StringTok{"free"}\NormalTok{))))}
\end{Highlighting}
\end{Shaded}
...
Permutation details:
  Number of permutations requested: 499
...
Model: rda(formula = spp ~ year + year:mowing + year:fertilizer + year:removal + Condition(plotid), data = env)
         Df Variance       F Pr(>F)   
RDA1      1    89.12 14.4173  0.002 **
RDA2      1    34.28  5.5458  0.004 **
RDA3      1    26.52  4.2900  0.004 **
RDA4      1     8.94  1.4458  0.640   
Residual 90   556.30                  
---
Signif. codes:  0 '***' 0.001 '**' 0.01 '*' 0.05 '.' 0.1 ' ' 1
\end{verbatim}
...
\begin{center}\includegraphics[width=0.7\linewidth]{ubc-vegan-study-group_files/figure-beamer/crayfish-pca-plot-1} \end{center}
...
\begin{Highlighting}[]
\KeywordTok{set.seed}\NormalTok{(}\DecValTok{1}\NormalTok{)}
\NormalTok{ctrl <-}\StringTok{ }\KeywordTok{how}\NormalTok{(}\DataTypeTok{nperm =} \DecValTok{499}\NormalTok{,}
            \DataTypeTok{within =} \KeywordTok{Within}\NormalTok{(}\DataTypeTok{type =} \StringTok{"none"}\NormalTok{),}
...
\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{summary}\NormalTok{(}\KeywordTok{eigenvals}\NormalTok{(m.re))}
\end{Highlighting}
\end{Shaded}
...
Importance of components:
                         RDA1    RDA2    RDA3    RDA4    RDA5    RDA6
Eigenvalue            0.62923 0.27057 0.21462 0.14135 0.11229 0.04669
Proportion Explained  0.09766 0.04199 0.03331 0.02194 0.01743 0.00725
Cumulative Proportion 0.09766 0.13965 0.17296 0.19490 0.21232 0.21957
                         RDA7    RDA8     RDA9     RDA10    PC1     PC2
Eigenvalue            0.03440 0.02704 0.006381 0.0003486 2.1635 0.60799
Proportion Explained  0.00534 0.00420 0.000990 0.0000500 0.3358 0.09436
Cumulative Proportion 0.22491 0.22911 0.230100 0.2301500 0.5659 0.66029
                          PC3     PC4     PC5     PC6     PC7     PC8
Eigenvalue            0.56047 0.51656 0.37495 0.22122 0.20516 0.15875
Proportion Explained  0.08699 0.08017 0.05819 0.03433 0.03184 0.02464
Cumulative Proportion 0.74728 0.82745 0.88564 0.91997 0.95181 0.97645
                          PC9     PC10
Eigenvalue            0.14770 0.004021
Proportion Explained  0.02292 0.000620
Cumulative Proportion 0.99938 1.000000
\end{verbatim}
...
\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{summary}\NormalTok{(}\KeywordTok{eigenvals}\NormalTok{(m.run))}
\end{Highlighting}
\end{Shaded}
...
Importance of components:
                        RDA1    RDA2    RDA3    RDA4    RDA5    RDA6
Eigenvalue            0.8541 0.31406 0.16791 0.13925 0.13278 0.08345
Proportion Explained  0.1722 0.06331 0.03385 0.02807 0.02677 0.01682
Cumulative Proportion 0.1722 0.23551 0.26936 0.29743 0.32420 0.34102
                         RDA7    RDA8    RDA9   RDA10    PC1     PC2
Eigenvalue            0.04738 0.04289 0.03904 0.00163 1.3137 0.41653
Proportion Explained  0.00955 0.00865 0.00787 0.00033 0.2648 0.08397
Cumulative Proportion 0.35058 0.35922 0.36709 0.36742 0.6323 0.71623
                          PC3     PC4     PC5     PC6     PC7     PC8
Eigenvalue            0.38322 0.27587 0.23785 0.17248 0.12150 0.11296
Proportion Explained  0.07726 0.05562 0.04795 0.03477 0.02449 0.02277
Cumulative Proportion 0.79349 0.84910 0.89705 0.93182 0.95632 0.97909
                          PC9     PC10
Eigenvalue            0.10158 0.002149
Proportion Explained  0.02048 0.000430
Cumulative Proportion 0.99957 1.000000
\end{verbatim}
...
\KeywordTok{set.seed}\NormalTok{(}\DecValTok{1}\NormalTok{)}
\NormalTok{ctrl <-}\StringTok{ }\KeywordTok{how}\NormalTok{(}\DataTypeTok{nperm =} \DecValTok{499}\NormalTok{,}
            \DataTypeTok{within =} \KeywordTok{Within}\NormalTok{(}\DataTypeTok{type =} \StringTok{"free"}\NormalTok{),}
...
Copyright \textcopyright (2015) Gavin L. Simpson \emph{Some Rights
Reserved}
